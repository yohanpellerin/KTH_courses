    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, current_total_steps, self.epsilon_initial, self.epsilon_final, self.annealing_timesteps, eps_type="constant")
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
            ...
            R_highest = max(R_highest, R_total)
            if len(N_previous_R) >= N:
                N_previous_R.pop(0)
            N_previous_R.append(R_total)
            print("Episode: {}, Highest Reward: {}, N_previous_R {}".format(episode, R_highest, N_previous_R))
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        Delta_p = self.final_p - self.initial_p
        p_t = self.initial_p + t * Delta_p / self.schedule_timesteps
        return p_t
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.2
